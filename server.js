...
app.post('/gif/data/gifno', async (req, res) => {
  const { gifno } = req.body;
  
  // Check if the no exists in the database
  const user = await GifDataModel.findOne({ gifno });
...
app.post('/won/no/win/data', async (req, res) => {
  const { no } = req.body;
  
  // Check if the no exists in the database
  const user = await WinnnModel.find({ no });
...
app.post('/won/rank/check', async (req, res) => {
  const { rank } = req.body;
  
  // Check if the email exists in the database
  const user = await WonModel.findOne({ rank });
